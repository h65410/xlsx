        ...
        complete(results) {
          ...
          results.data
            ...
            .forEach((v: any) => {
              ...
              const t = v["Dest Channel Extension"].toString();
              if (!_data[id]) _data[id] = { id };
              if (
                !_data[id]["d"] ||
                !(date.getDateDiff(d, _data[id]["d"], "minutes") > 0)
              ) {
                _data[id]["d"] = d;
              }
              if (!_data[id][t]) {
                _data[id][t] = v["Disposition"];
              }
            });
          ...
          Object.keys(_data).forEach((k) => {
            if (!_data[k]["1080"]) {
              _data[k]["1080"] = "NO ANSWER";
              _data[k]["l"] = "1080";
            }
            if (!_data[k]["1084"]) {
              _data[k]["1084"] = "NO ANSWER";
              _data[k]["l"] = "1084";
            }
          });

          data.push(
            Object.values(_data).filter(
              (v) => !(v["1080"] == "BUSY" && v["1084"] == "BUSY")
            )
          );
